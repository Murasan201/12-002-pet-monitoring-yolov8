...
- **外乱（ケーブル）対策**: カメラモジュール/給電ケーブル等のテンションは追跡制御を不安定化させるため、必ず余長と固定（ストレインリリーフ）を確保する
...
5. 検出成功
   ├─ Yes → P制御で角度更新
   └─ No  → 一時的に保持（ロスト猶予内は追跡継続）
    ↓
6. 未検出がロスト猶予を超えたら追跡終了 → 待機して再スキャン
```
...
| パラメータ | デフォルト値 | 説明 |
|----------|------------|------|
| tracking_duration | 8.0秒 | 追跡ループの区切り時間（検出が続く限り自動延長） |
| tracking_fps | 5 Hz | 追跡ループの更新周波数 |
| TRACK_LOST_TIMEOUT | 10.0秒 | 検出が途切れても追跡を続ける猶予（この時間を超えたらロスト） |
| RESCAN_DELAY_SECONDS | 2.0秒 | ロスト後に再スキャンへ入る前の待機時間 |

**環境変数の例**:
```bash
# 検出の瞬断が多い環境では長めに
TRACK_LOST_TIMEOUT=10.0
# ロスト後に落ち着くのを待ってからスキャン再開
